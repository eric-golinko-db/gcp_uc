...
The "Getting Started" docs can be found at https://docs.gcp.databricks.com/dev-tools/bundles/mlops-stacks.html.
...
| ML Resources as Code | ML pipeline resources (training and batch inference jobs with schedules, etc) configured and deployed through [databricks CLI bundles](https://docs.gcp.databricks.com/dev-tools/cli/bundle-cli.html)                                                                                              |

contained in the following files:

```
gcp_uc        <- Root directory. Both monorepo and polyrepo are supported.
│
├── gcp_uc       <- Contains python code, notebooks and ML resources related to one ML project. 
│   │
...
To use Repos, [set up git integration](https://docs.gcp.databricks.com/repos/repos-setup.html) in your dev workspace.

If the current project has already been pushed to a hosted Git repo, follow the
[UI workflow](https://docs.gcp.databricks.com/en/repos/git-operations-with-repos.html#clone-a-repo-connected-to-a-remote-repo)
...
* Follow the [UI workflow](https://docs.gcp.databricks.com/en/repos/git-operations-with-repos.html#clone-a-repo-connected-to-a-remote-repo)
...
* Run `databricks configure --profile gcp_uc-dev --token --host <your-dev-workspace-url>`, passing the URL of your dev workspace.
  This should prompt you to enter an API token
* [Create a personal access token](https://docs.gcp.databricks.com/dev-tools/auth/pat.html)
...
* From within the root directory of the current project, use the [dbx sync](https://dbx.readthedocs.io/en/latest/guides/python/devloop/mixed/#using-dbx-sync-repo-for-local-to-repo-synchronization) tool to copy code files from your local machine into the Repo by running
  `dbx sync repo --profile gcp_uc-dev --source . --dest-repo your-repo-name`, where `your-repo-name` should be the last segment of the full repo name (`/Repos/username/your-repo-name`)
