...
MLOps Stacks ML resources are configured and deployed through [databricks CLI bundles](https://docs.gcp.databricks.com/dev-tools/cli/bundle-cli.html).
The bundle setting file must be expressed in YAML format and must contain at minimum the top-level bundle mapping.

The databricks CLI bundles top level is defined by file `gcp_uc/databricks.yml`.
...
ML Resource Configurations in this directory:
 - model workflow (`gcp_uc/resources/model-workflow-resource.yml`)
 - batch inference workflow (`gcp_uc/resources/batch-inference-workflow-resource.yml`)
 - monitoring resource and workflow (`gcp_uc/resources/monitoring-resource.yml`)
 - feature engineering workflow (`gcp_uc/resources/feature-engineering-workflow-resource.yml`)
 - model definition and experiment definition (`gcp_uc/resources/ml-artifacts-resource.yml`)
...
If you want to use different catalog names, please update the `targets` declared in the `gcp_uc/databricks.yml` and `gcp_uc/resources/ml-artifacts-resource.yml` files.
...
|-------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------|-------------------------------------|------------------------------------------------|
| dev         | The `dev` deployment target is used by ML engineers to deploy ML resources to development workspace with `dev` configs. The config is for ML project development purposes.                                                           | dev workspace        | dev-gcp_uc-model     | /dev-gcp_uc-experiment     |
| staging     | The `staging` deployment target is part of the CD pipeline. Latest main content will be deployed to staging workspace with `staging` config.                                                             | staging workspace    | staging-gcp_uc-model | /staging-gcp_uc-experiment |
| prod        | The `prod` deployment target is part of the CD pipeline. Latest release content will be deployed to prod workspace with `prod` config.                                                                      | prod workspace       | prod-gcp_uc-model    | /prod-gcp_uc-experiment    |
| test        | The `test` deployment target is part of the CI pipeline. For changes targeting the main branch, upon making a PR, an integration test will be triggered and ML resources deployed to the staging workspace defined under `test` deployment target. | staging workspace    | test-gcp_uc-model    | /test-gcp_uc-experiment    |
...
1. Follow [databricks CLI](https://docs.gcp.databricks.com/dev-tools/cli/databricks-cli.html) to download and set up the databricks CLI locally.
2. Complete the `TODO` in `gcp_uc/databricks.yml` to add the dev workspace URI under `targets.dev.workspace.host`.
3. [Create a personal access token](https://docs.gcp.databricks.com/dev-tools/auth/pat.html)
...
Following this, update the batch_inference_job base parameters in `gcp_uc/resources/batch-inference-workflow-resource.yml` to pass
...
To get started, open `gcp_uc/resources/batch-inference-workflow-resource.yml`.  The file contains the ML resource definition of a batch inference job, like:
...
resources:
  jobs:
    batch_inference_job:
      name: ${bundle.target}-gcp_uc-batch-inference-job
      tasks:
...
The example above defines a Databricks job with name `${bundle.target}-gcp_uc-batch-inference-job`
that runs the notebook under `gcp_uc/deployment/batch_inference/notebooks/BatchInference.py` to regularly apply your ML model for batch inference. 
...
We specify a `batch_inference_job` under `resources/jobs` to define a databricks workflow with internal key `batch_inference_job` and job name `{bundle.target}-gcp_uc-batch-inference-job`.
...
`test-gcp_uc-batch-inference-job`. During the deployment of the `staging` environment config, the job name will be
`staging-gcp_uc-batch-inference-job`.
